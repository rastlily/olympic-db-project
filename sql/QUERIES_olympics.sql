...
-- 8. How did each country rank by total medals in each Olympic year?
...
WITH CountryMedals AS (
    ...
    JOIN countries c ON ae.NOC = c.NOC
    WHERE ae.Medal IS NOT NULL
    GROUP BY ae.Year, c.Country
)
SELECT 
    Year,
    Country,
    Total_Medals,
    RANK() OVER (PARTITION BY Year ORDER BY Total_Medals DESC) AS Country_Rank
FROM CountryMedals
ORDER BY Year, Country_Rank;

-- 9. How has each country's total medal count accumulated over time?

WITH YearlyMedals AS (
    SELECT 
        ae.Year,
        c.Country,
        COUNT(ae.Medal) AS Medals_Per_Year
    FROM athlete_events ae
    JOIN countries c ON ae.NOC = c.NOC
    WHERE ae.Medal IS NOT NULL
    GROUP BY ae.Year, c.Country
...
SELECT 
    ...
    Year,
    Medals_Per_Year,
    SUM(Medals_Per_Year) OVER (PARTITION BY Country ORDER BY Year) AS Cumulative_Medals
FROM YearlyMedals
